...
# Build
xcodebuild -project ZoomacIt.xcodeproj -scheme ZoomacIt -configuration Debug build

# Run tests (14 tests)
xcodebuild -project ZoomacIt.xcodeproj -scheme ZoomacItTests test

# Launch
open ~/Library/Developer/Xcode/DerivedData/ZoomacIt-*/Build/Products/Debug/ZoomacIt.app
```

The `.xcodeproj` is generated by **xcodegen** from `project.yml`. Both are committed — regenerate only when `project.yml` changes.

## Architecture (5 layers, top→bottom dependency)

| Layer | Directory | Key Files |
|-------|-----------|-----------|
| **App** | `ZoomacIt/App/` | `main.swift` (explicit entry point, NOT `@main`), `AppDelegate.swift`, `StatusBarController.swift` |
| **Core** | `ZoomacIt/Core/` | `HotkeyManager.swift` (Carbon `RegisterEventHotKey`), `PermissionManager.swift` |
| **Overlay** | `ZoomacIt/Overlay/` | `OverlayWindow.swift`, `OverlayWindowController.swift` (ScreenCaptureKit) |
| **Draw** | `ZoomacIt/Draw/` | `DrawingCanvasView.swift` (core — 3-layer compositing), renderers, `StrokeManager.swift` |
| **Models/Utils** | `ZoomacIt/Models/`, `ZoomacIt/Utilities/` | `DrawingState.swift`, `Stroke.swift`, extensions |
...
### LSUIElement App
`Info.plist` has `LSUIElement=true` (no Dock icon). To show modal alerts, temporarily switch to `.regular` activation policy, then revert to `.accessory` (see `PermissionManager.requestRequiredPermissions()`).
...
### Concurrency
- `@MainActor` on all UI classes (`AppDelegate`, `StatusBarController`, `PermissionManager`, `OverlayWindowController`)
